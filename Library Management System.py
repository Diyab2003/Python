...
def save():
    ...
    if bkid.get() == '' or bkname.get() == '' or author.get() == '':
        ...
        cardid.set("N/A")
        s = "INSERT INTO bookentry (bkid, bkname, author_name, status, cardid) VALUES (%s, %s, %s, %s, %s)"
        cur.execute(s, (bkid.get(), bkname.get(), author.get(), status.get(), cardid.get()))
        mycon.commit()
...
def issuer_card():
    ...
    if not Cid:
        msgbox.showerror('Error!', 'Issuer ID cannot be empty')
    else:
        return Cid
...
def update_record():
    def update():
        if status.get() == 'Issued':
            ...
            cardid.set('N/A')
        s = "UPDATE bookentry SET bkname = %s, author_name = %s, status = %s, cardid = %s WHERE bkid = %s"
        cur.execute(s, (bkname.get(), author.get(), status.get(), cardid.get(), bkid.get()))
        mycon.commit()
        clear()
        edit.destroy()
    ...
    s = "SELECT * FROM bookentry WHERE bkid = %s" 
    cur.execute(s, (selection[0],))
    data = cur.fetchone()
    if data:
        bkid.set(data[0])
        bkname.set(data[1])
...
def delete():
    ...
    values = tree.item(current_item)
    selection = values["values"]
    a = "DELETE FROM bookentry WHERE bkid = %s"
    cur.execute(a, (selection[0],))
    mycon.commit()
    tree.delete(current_item)
    msgbox.showinfo('Done', 'The record was successfully deleted.')

lt_frame = Frame(root, bg="light blue", relief="solid")
lt_frame.place(x=0, y=35, width=400, height=500)
rt_frame = Frame(root, bg="#ffbb33")
rt_frame.place(x=400, y=35, width=600, height=400)
btm_frame = Frame(root, bg="orange")
btm_frame.place(x=400, y=435, width=600, height=65)
...
e1 = Entry(lt_frame, width=25, textvariable=bkid, font=("Bookman old style", 14, "bold"), bg="light yellow")
e1.place(x=30, y=90)
Label(lt_frame, text="Book Name: ", font=("Bookman old style", 18, "bold"), fg="white", bg="orange").place(x=50, y=130)
Entry(lt_frame, width=25, textvariable=bkname, font=("Bookman old style", 14, "bold"), bg="light yellow").place(x=30, y=170)
Label(lt_frame, text="Author Name: ", font=("Bookman old style", 18, "bold"), fg="white", bg="orange").place(x=50, y=210)
Entry(lt_frame, width=25, textvariable=author, font=("Bookman old style", 14, "bold"), bg="light yellow").place(x=30, y=250)
...
tree.place(y=65, x=0, height=370, width=600)
